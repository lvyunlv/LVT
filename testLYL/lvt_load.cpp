...
int main(int argc, char** argv) {
    ...
    if (argc < 5) {
        std::cout << "Format: <PartyID> <port> <num_parties> <function>" << std::endl;
        return 0;
    }
    ...
    LVT<MultiIOBase>::initialize(
        function,
        "../../build/bin/table_" + function + ".txt",
        lvt_raw,
        "lvt_" + function + "_instance.bin",
        num_party,
        party,
        io.get(),
        &pool,
    ...
    std::string output_file = "../../TestLYL/Output/Output-" + function + "-P" + std::to_string(party) + ".txt";
    {
