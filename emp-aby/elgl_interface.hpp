...
namespace emp {
    ...
    class ELGL{
        ...
        public:
            ...
            // 证明： (g, h, g^x, h^x)
            void DecProof(const ELGL_PK global_pk, std::map<std::string, Fr>& P_to_m, std::stringstream& commitment, std::stringstream& response, std::stringstream& encMap, vector<int64_t> table, unsigned table_size,vector<BLS12381Element>& EncTable_c0, vector<BLS12381Element>& EncTable_c1){
                ...
                // if (global_pk.get_pk() != BLS12381Element(sk.get_sk())){
                //     std::cout << "error: global_pk != sk" << std::endl;
                // }
            ...
            void DecVerify(std::stringstream& commitment, const ELGL_PK global_pk, std::stringstream& response, std::stringstream& encMap, vector<BLS12381Element>& EncTable_c0, vector<BLS12381Element>& EncTable_c1, unsigned table_size){
                // verify
                ExpProof proof(global_pk, table_size);
