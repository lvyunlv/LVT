...
size_t RangeProver::NIZKPoK(RangeProof& P, std::stringstream& ciphertexts, std::stringstream& cleartexts, const ELGL_PK& pk, const std::vector<BLS12381Element>& g1, const std::vector<BLS12381Element>& y3, const std::vector<BLS12381Element>& y2, const std::vector<Plaintext>& x, const Plaintext& ski){
                // TODO: check if allocate is enough
    // size_t allocate = (10 * P.n_proofs + 2) * G1::getSerializedByteSize();
    // ciphertexts.resize_precise(allocate);
    // ciphertexts.reset_write_head();

    // allocte memory for cleartexts
    // size_t allocate2 = (P.n_proofs * 2 + 1) * Fr::getByteSize();
    ...
    // pack p
    P.get_bound().save(ciphertexts);


    for (unsigned int i = 0; i < y3.size(); i++){
        std::cout << "prover c[" << i << "]: " << std::endl;
