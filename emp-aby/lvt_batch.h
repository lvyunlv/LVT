...
void LVT<IO>::generate_shares(vector<Plaintext>& lut_share, Plaintext& rotation, vector<int64_t> table) {
    ...
    if (party == ALICE) {
        ...
        vector<std::stringstream> comm(tb_size), response(tb_size), encMap(tb_size);
        //time
        ...
        vector<BLS12381Element> y2(tb_size);
        vector<Plaintext> x(tb_size);
        vector<Plaintext> r1(tb_size);
        BLS12381Element pk_ = global_pk.get_pk();

        for (size_t i = 0; i < tb_size; i++){
            // res.emplace_back(pool->enqueue([&]() {
                ExpProof proof(global_pk, tb_size);
                //y1 = g^r, y2 = gpk^r
                x[i] = Plaintext(Fr(this->table[i]));
                r1[i].set_random();
                c0[i] = BLS12381Element(r1[i].get_message());
                y2[i] = global_pk.get_pk() * r1[i].get_message();
                c1[i] =  y2[i] + BLS12381Element(x[i].get_message());
                c1[i].pack(encMap[i]);

                ExpProver prover(proof);
                // size_t ExpProver::NIZKPoK(ExpProof& P, std::stringstream& ciphertexts, std::stringstream&  cleartexts, const BLS12381Element& g1, const BLS12381Element& y1, const BLS12381Element& y2, const Plaintext& x, int i, ThreadPool* pool)
                prover.NIZKPoK(proof, comm[i], response[i], pk_, c0[i], y2[i], r1[i], 1, pool);
            // }));
        }
        ...
        for (size_t i = 0; i < tb_size; i++){
            ...
            std::string comm_raw = comm[i].str();
            // comm_ << base64_encode(comm_raw);
            std::string response_raw = response[i].str();
            // response_ << base64_encode(response_raw);
            std::string encMap_raw = encMap[i].str();
            ...
            combined << base64_encode(comm_raw) << "\n"
                    << base64_encode(response_raw) << "\n"
                    << base64_encode(encMap_raw) << "\n";
            elgl->serialize_sendall_(combined);
        }      
        ...
        BLS12381Element pk_ = global_pk.get_pk();
        vector<std::stringstream> comm_data(tb_size), response_data(tb_size), encMap_data(tb_size);
        ...
        for (size_t i = 0; i < tb_size; i++){
            ...
            std::stringstream combined;
            elgl->deserialize_recv_(combined, ALICE);

            std::string line;
            std::getline(combined, line); response_data[i] << base64_decode(line);
            std::getline(combined, line); comm_data[i] << base64_decode(line);
            std::getline(combined, line); encMap_data[i] << base64_decode(line);
            
            // comm_raw = comm.str();
            // response_raw = response.str();
            // encMap_raw = encMap.str();
            
            // comm_data[i] << base64_decode(comm_raw);
            // response_data[i] << base64_decode(response_raw);
            // encMap_data[i] << base64_decode(encMap_raw);
        }
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed = end - start;
        ...
        for (size_t i = 0; i < tb_size; i++){
            // res.emplace_back(pool->enqueue([&]() {
                ...
                verifier.NIZKPoK(pk_, c0[i], y2, comm_data[i], response_data[i], pool, 1);
                c1[i].unpack(encMap_data[i]);
            // }));
        }
        // for (auto & f : res) f.get();
        // res.clear();
